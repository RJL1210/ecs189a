...
def analyze_distribution_transformation(D: Dict[str, float], p_samples: List[int], epsilon: float) -> Dict[str, Dict]:
    ...
    print(f"Sample sizes - Original: {len(p_samples)}, Mapped: {len(mapped_samples)}")
    ...
    print(f"Original range: {min(original_dist.values()):.6f} to {max(original_dist.values()):.6f}")
    print(f"Final range: {min(final_dist.values()):.6f} to {max(final_dist.values()):.6f}")
...
def visualize_batch_results(test_dir: str, sample_dir: str, epsilon: float):
    ...
    for dist_file in distribution_files:
        ...
        print("Close samples distribution shape:", len(close_results['original']), len(close_results['final']))
        ...
        print("Far samples distribution shape:", len(far_results['original']), len(far_results['final']))
